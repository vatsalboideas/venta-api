...
const ALLOWED_PERIODS = ["day", "month"] as const;
...
export async function getRevenueTrend(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const bucket = rawPeriod as Period;
    ...
    return res.json(
      rows.map((r) => ({ bucket: r.bucket, revenue: Number(r.revenue) })),
    );
  } catch (err) {
    return next(err);
  }
...
export async function getConversionRate(_req: Request, res: Response, next: NextFunction) {
  try {
